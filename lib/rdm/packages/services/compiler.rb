      ...
      class Compiler
        class << self
          def compile(compile_path:, project_path:, package_name:)
            Rdm::Packages::Services::Compiler.new(
        ...
        attr_reader :compile_path, :project_path, :package_name, :source
        ...
        def compile
          ...
          Dir["#{compile_path}/**/*"].grep(/\/spec$/).each do |spec_dir|
            FileUtils.rm_rf(spec_dir)
          end
          Dir["#{compile_path}/**/*.log"].each do |filepath|
            FileUtils.rm(filepath)
          end
          %w(.gitignore .byebug_history .irbrc .rspec).each do |filename|
            Dir["#{compile_path}/**/#{filename}"].each do |filepath|
              FileUtils.rm(filepath)
            end
          end
